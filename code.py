import tkinter
from textblob import TextBlob from tkinter import *
import matplotlib.pyplot as plt; plt.rcdefaults() 
import numpy as np
import pandas as pd
from string import punctuation from nltk.corpus 
import stopwords import os

main = tkinter.Tk()
main.title("Predicting Crypto Currency Fluctuations Using Machine Learning on Tweets") #designing main screen
main.geometry("1300x1200")

global filename tweets_list = [] clean_list = [] global pos, neu, neg

def tweetCleaning(doc): 
    tokens = doc.split()
    table = str.maketrans('', '', punctuation) tokens = [w.translate(table) for w in tokens]
...
#here upto for word based return tokens
#function to upload tweeter profile global filename
#filename = filedialog.askopenfilename(initialdir="dataset")

def upload(): 
    filename='F:/python/CryptoCurrencyMachineLearning/dataset/bitcoin_tweets.csv' 
...
train = pd.read_csv('F:/python/CryptoCurrencyMachineLearning/dataset/bitcoin_tweets.csv',encoding='is o-8859-1')
...
if blob.polarity <= 0.2:
    neg = neg + 1 text.insert(END,tweet+"\n")
...
def graph():
    label_X = [] category_X = [] text.delete('1.0', END)
    ...
    +'%\n')
    label_X.append('Positive') label_X.append('Negative') label_X.append('Neutral') category_X.append(pos) category_X.append(neg) category_X.append(neu)
    plt.pie(category_X,labels=label_X,autopct='%1.1f%%') plt.title('Women Saftey & Sentiment Graph') plt.axis('equal')
    plt.show()
...
def negative(): 
    os.system('negative.py')

font = ('times', 16, 'bold')

title = Label(main, text='Predicting Crypto Currency Fluctuations Using Machine Learning on Tweets')
...
pgraphButton = Button(main, text="Possitive grap", command=Positive) 
pgraphButton.place(x=950,y=150)
pgraphButton.config(font=font1)
pgraphButton = Button(main, text="Negative grap", command=negative) 
pgraphButton.place(x=1100,y=150)
pgraphButton.config(font=font1)
